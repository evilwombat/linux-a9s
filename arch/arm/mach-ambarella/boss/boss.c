...
#if (CHIP_REV == S3)
	amba_writel(VIC4_REG(VIC_INTEN_OFFSET), boss->int_mask[3] & boss->guest_int_en[3]);
#endif
#endif
...
#if (CHIP_REV == S3)
	amba_writel(VIC4_REG(VIC_INTEN_CLR_OFFSET), boss->int_mask[3]);
#endif
#endif
...
#if (CHIP_REV == S3)
			case 3:
				amba_writel(VIC4_REG(VIC_INTEN_OFFSET),
					    1 << (irq % 32));
				break;
#endif
			default:
...
#if (CHIP_REV == S3)
				case 3:
					amba_writel(VIC4_REG(VIC_INTEN_CLR_OFFSET),
						    1 << (irq % 32));
					break;
#endif
				default:
